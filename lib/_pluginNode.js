...
process.on('message', function(m) {
    ...
    case 'import':
        ...
        break;
    case 'importJailed':
        importScriptJailed(m.url, m.options);
        break;
    case 'execute':
        execute(m.code, m.options);
        break;
    case 'message':
 ...
 * @param {String} url of the script to load
 */
var importScriptJailed = function(url, options) {
    var sCb = function() {
       process.send({type: 'importSuccess', url: url});
    }
    ...
    var run = function(code) {
        executeJailed(code, url, sCb, fCb, options);
    }
...
var execute = function(code, options) {
    ...
    executeJailed(code, 'DYNAMIC PLUGIN', sCb, fCb, options);
}
 ...
 * @param {Function} fCb
 */
var executeJailed = function(code, url, sCb, fCb, options) {
    var vm  = require('vm');
    var sandbox = {};
    var requireExposed = false;

    if ( options.secure ) {
        requireExposed = options.expose.indexOf('require') === -1 ? false : true;
        for (var i = 0; i < options.expose.length; i++) {
            if ( options.expose[i] !== 'require' ) {
                sandbox[options.expose[i]] = global[options.expose[i]];
            }
        }
        ...
        try {
            if ( requireExposed ) {
                vm.runInNewContext(code, vm.createContext(sandbox), url)(require);
            }
            else {
    ...
    // Run the code insecurely
    else {
        code = '(function(require) {\n' + code + '\n});';
        try {
            vm.runInThisContext(code,  url)(require);
            sCb();
        } catch (e) {
            printError(e.stack);
            fCb();
